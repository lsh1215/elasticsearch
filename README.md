...
이 저장소는 대화 기반 학습 내용을 다음 구조로 정리합니다.

| Path | Role |
|------|------|
| `docs/study-log/` | 날짜별 공부 흐름, 의미 있는 질문과 요약, 실험 결과, 남은 의문 |
| `docs/concepts/` | 개념별 최종 정리. 근원탐색 내용과 References 포함 |
| `docs/deep-dives/` | 여러 개념을 관통하는 큰 질문, 비교, 역사, 설계 배경 |
| `docs/labs/` | 명령어, 실행 결과, 에러, 해결 과정, 배운 점 |
| `omx_wiki/` | Codex가 재사용할 수 있는 검증된 핵심 지식. 이 디렉토리는 사용자가 Obsidian에서 직접 생성 |
